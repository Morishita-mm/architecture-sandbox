...
`npm run dev`のチャットは、APIキーやバックエンドなしで定型応答を返します。送信中の表示、会話の継続、保存を確認できます。定型応答は正式な要件や仕様変更として記録しません。ビルド済みアプリでは常に実際のAPIを使います。

開発サーバーで実際のAIとの会話を試す場合は、`VITE_LOCAL_CHAT=false`を環境変数またはルートの`.env.local`に設定してください。評価にはバックエンドが必要です。
